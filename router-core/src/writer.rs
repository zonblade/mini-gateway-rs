...
/// Detects the init system being used by the operating system
pub fn detect_init_system() -> Option<String> {
    #[cfg(target_os = "windows")]
    {
        // Windows uses its own service manager
        return Some("windows-services".to_string());
    }

    #[cfg(target_os = "macos")]
    {
        // macOS uses launchd
        return Some("launchd".to_string());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Check for OpenRC
        let openrc_check = Command::new("rc-status").arg("--version").output();
        if openrc_check.is_ok() && openrc_check.unwrap().status.success() {
            return Some("openrc".to_string());
        }

        // Check for SysVinit
        if Path::new("/etc/init.d").exists() && Path::new("/etc/inittab").exists() {
            return Some("sysvinit".to_string());
        }

        // Check for Upstart
        if Path::new("/sbin/initctl").exists() {
            let upstart_check = Command::new("/sbin/initctl").arg("--version").output();
            if upstart_check.is_ok() && upstart_check.unwrap().stdout.starts_with(b"initctl") {
                return Some("upstart".to_string());
            }
        }
    ...
    // Unknown or unsupported init system
    None
}
...
pub fn writer_start() {
    // Detect the init system and log it
    if let Some(_) = detect_init_system() {
        // if the init system is detected, use default logging
        std::env::set_var("RUST_LOG", "info");
        env_logger::init();
    } else {
