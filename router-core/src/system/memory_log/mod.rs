...
// Debug helper to get current function name
...
impl QueueControl {
    ...
    pub fn force_reset(&self, capacity: usize) {
        ...
        let old_write_idx = self.write_index.load(Ordering::Acquire);
        let old_read_idx = self.read_index.load(Ordering::Acquire);
...
impl SharedMemoryProducer {
    ...
    pub fn create_with_options(
        ...
        let fd = unsafe {
            ...
            let mut attempts = 0;
            let max_attempts = 3;
            let mut last_err = Error::new(ErrorKind::Other, "Unknown error");
    ...
    pub fn create_with_capacity(name: &str, min_capacity: usize, fresh_start: bool, 
                              overflow_policy: OverflowPolicy) -> io::Result<Self> {
            ...
            if count >= capacity {
                match self.overflow_policy {
                    OverflowPolicy::Block => {
                        ...
                        (*self.control).overflow_count.fetch_add(1, Ordering::Relaxed);
...
impl LogProducer {
    ...
    pub fn new_with_options(
        ...
        let shm = match SharedMemoryProducer::create_with_options(
            ...
            Err(e) => {
                if size > 100 * 1024 * 1024 && (e.kind() == ErrorKind::PermissionDenied || 
                                               e.kind() == ErrorKind::AddrNotAvailable ||
                                               e.kind() == ErrorKind::OutOfMemory) {
                    return Self::new_with_options(name, 100 * 1024 * 1024, fresh_start, overflow_policy);
                }
        ...
        if total_size_of_payload > max_payload_in_shm_entry {
            ...
            if available_for_message_text == 0 {
                ...
                if header_size + suffix_len <= max_payload_in_shm_entry {
                    ...
                    return Err(Error::new(ErrorKind::InvalidInput, "Log entry too small for header and truncation suffix."));
                }
            } else {
                ...
                if cut_at == 0 && !message.is_empty() {
                    ...
                    if header_size + suffix_len <= max_payload_in_shm_entry {
                        ...
                        return Err(Error::new(ErrorKind::InvalidInput, "Log entry too small for even one char and truncation suffix."));
                    }
                } else {
...
pub unsafe fn proxy_logger() -> io::Result<&'static LogProducer> {
    ...
    match &GLOBAL_LOG_PROXY {
        ...
        None => {
            Err(Error::new(ErrorKind::Other, "Failed to initialize proxy logger"))
        },
    }
}
...
pub unsafe fn gateway_logger() -> io::Result<&'static LogProducer> {
    ...
    match &GLOBAL_LOG_GATEWAY {
        ...
        None => {
            Err(Error::new(ErrorKind::Other, "Failed to initialize gateway logger"))
        },
    }
}
...
pub unsafe fn log_proxy(level: u8, message: &str) -> io::Result<()> {
    match proxy_logger() {
        Ok(logger) => {
            match logger.log(level, message) {
                Ok(_) => {
                    Ok(())
                },
                Err(e) => {
