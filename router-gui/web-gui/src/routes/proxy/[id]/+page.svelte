<script lang="ts">
    ...
    function apiToUiProxy(apiProxy: Proxy): {
        proxy: UIProxy;
        ...
        if (
            "tls_domains" in apiProxy &&
            Array.isArray(apiProxy.tls_domains) &&
            apiProxy.tls_domains.length > 0
        ) {
            // Get domains from tls_domains property
            domains.push(
                ...apiProxy.tls_domains.map((domain, index) => ({
                ...
                domainConfigs.length > 0
                    ? domainConfigs.map((config) => {
                          ...
                          return {
                              sni: config.domain,
                              tls: config.useTls,
                              tls_pem: config.useTls
                                  ? config.certPem || null
                                  : null,
                              tls_key: config.useTls
                                  ? config.certKey || null
                                  : null,
                              tls_autron: config.useTls
                                  ? config.autoTls
                                  : false,
                              proxy_id: isNewDomain
                                  ? ""
                                  : config.proxy_id || uiProxy.id,
    ...
    async function loadGwNodes() {
        ...
        try {
            ...
            console.log(
                `Loaded ${gwNodes.length} gateway nodes for proxy ${proxyId}`,
            );
        } catch (error) {
    ...
    async function loadProxy() {
        try {
            ...
            if ("proxy" in apiProxyData && "domains" in apiProxyData) {
                ...
                const { proxy: loadedProxy, domains } = apiToUiProxy(
                    apiProxyData as Proxy,
                );
                proxy = loadedProxy;
    ...
    <div class="container mx-auto max-w-[900px] px-4 py-8">
        <div class="bg-white dark:bg-gray-800 border-1 border-gray-200 dark:border-gray-700 p-6">
            ...
            <form on:submit|preventDefault={saveProxy} class="space-y-6">
                ...
                <DomainConfiguration bind:domainConfigs />
