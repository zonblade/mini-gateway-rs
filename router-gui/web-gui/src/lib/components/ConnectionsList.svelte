<script lang="ts">
  ...
  function handleStartEdit(connection: Connection) {
    editingConnection = { ...connection };
    isEditingConnection = true;
  }

  function handleUpdateConnection() {
    if (!editingConnection || !editingConnection.name || !editingConnection.host) {
      return;
    }
    connections.updateConnection(editingConnection.id, {
      name: editingConnection.name,
      protocol: editingConnection.protocol,
      host: editingConnection.host,
      port: editingConnection.port,
      subpath: editingConnection.subpath
    });
...
<div class="h-full flex flex-col">
        ...
        >
          <div class="flex justify-between items-start">
            ...
            <div class="flex items-center gap-2">
              <button
                on:click|stopPropagation={() => handleStartEdit(connection)}
  ...
  <!-- Popup Modal for Editing Connection -->
  {#if isEditingConnection && editingConnection}
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      <div
        class="absolute inset-0 bg-gray-500/30 dark:bg-gray-900/50 backdrop-blur-sm transition-opacity"
        on:click={() => { isEditingConnection = false; editingConnection = null; }}
        on:keydown={(e) => e.key === 'Escape' && (isEditingConnection = false, editingConnection = null)}
        tabindex="-1"
      ></div>
      ...
      >
        <div class="flex justify-between items-center mb-4">
          <h4 class="text-base font-normal">Edit Connection</h4>
          <button
            on:click={() => { isEditingConnection = false; editingConnection = null; }}
            class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"
            aria-label="Close modal"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a 1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
            </svg>
          </button>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div>
            <label for="edit-protocol" class="block text-sm mb-1">Protocol</label>
            <select
              id="edit-protocol"
              bind:value={editingConnection.protocol}
              class="w-full px-3 py-2 border-b border-gray-300 dark:border-gray-700 bg-transparent focus:outline-none focus:border-gray-500 dark:focus:border-gray-400"
            >
              <option value="http">HTTP</option>
              <option value="https">HTTPS</option>
          ...
          <div>
            <label for="edit-host" class="block text-sm mb-1">Host</label>
            <input
              type="text"
              id="edit-host"
              bind:value={editingConnection.host}
              class="w-full px-3 py-2 border-b border-gray-300 dark:border-gray-700 bg-transparent focus:outline-none focus:border-gray-500 dark:focus:border-gray-400"
              placeholder="localhost"
            />
          </div>
          <div>
            <label for="edit-port" class="block text-sm mb-1">Port</label>
            <input
              type="number"
              id="edit-port"
              bind:value={editingConnection.port}
              class="w-full px-3 py-2 border-b border-gray-300 dark:border-gray-700 bg-transparent focus:outline-none focus:border-gray-500 dark:focus:border-gray-400"
              placeholder="8080"
            />
          </div>
          <div>
            <label for="edit-subpath" class="block text-sm mb-1">Subpath</label>
            <input
              type="text"
              id="edit-subpath"
              bind:value={editingConnection.subpath}
              class="w-full px-3 py-2 border-b border-gray-300 dark:border-gray-700 bg-transparent focus:outline-none focus:border-gray-500 dark:focus:border-gray-400"
              placeholder="/api/v1"
            />
          </div>
          <div class="flex justify-end space-x-3 mt-6">
            ...
            <button
              on:click={handleUpdateConnection}
              class="px-3 py-2 text-sm bg-gray-800 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 text-white"
            >
              Save
            </button>
