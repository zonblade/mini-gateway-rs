...
impl CertbotConfig {
    ...
    pub fn get_config_dir(&self) -> String {
        self.config_dir
            .clone()
            .unwrap_or_else(|| "/etc/letsencrypt".to_string())
    }
    ...
    pub fn get_work_dir(&self) -> String {
        self.work_dir
            .clone()
            .unwrap_or_else(|| "/var/lib/letsencrypt".to_string())
    }
    ...
    pub fn get_logs_dir(&self) -> String {
        self.logs_dir
            .clone()
            .unwrap_or_else(|| "/var/log/letsencrypt".to_string())
    }
}
...
impl CertbotManager {
    ...
    pub fn ensure_certificate(&self, domain: &str) -> Result<CertificateInfo> {
        ...
        if !Path::new(&cert_info.path_cert).exists() {
            println!(
                "Certificate not found for domain: {}, creating new certificate",
                domain
            );
            ...
            match self.check_certificate_expiry(&cert_info.path_cert) {
                Ok(expires_at) => {
                    ...
                    if days_until_expiry <= 30 {
                        println!(
                            "Certificate for domain {} expires in {} days, renewing",
                            domain, days_until_expiry
                        );
                        self.renew_certificate(domain)?;
                    } else {
                        println!(
                            "Certificate for domain {} is valid for {} more days",
                            domain, days_until_expiry
                        );
                    }
                }
                Err(_) => {
                    println!(
                        "Could not check certificate expiry for domain: {}, attempting renewal",
                        domain
                    );
                    self.renew_certificate(domain)?;
                }
    ...
    pub fn detect_certificate_origin(&self, domain: &str) -> Option<String> {
        ...
        let cert_exists =
            Path::new(&cert_info.path_cert).exists() && Path::new(&cert_info.path_key).exists();
    ...
    pub fn create_certificate_interactive(&self, domain: &str) -> Result<()> {
        ...
        println!(
            "Creating SSL certificate interactively for domain: {}",
            domain
        );
        ...
        let status = cmd.status().map_err(|e| {
            CertbotError::CommandFailed(format!("Failed to execute command: {}", e))
        })?;
        ...
        if status.success() {
            ...
            Err(CertbotError::CommandFailed(
                "Interactive certificate creation failed".to_string(),
            ))
        }
    }
    ...
    fn check_certificate_expiry(&self, cert_path: &str) -> Result<DateTime<Utc>> {
        let output = Command::new("openssl")
            ...
            .output()
            .map_err(|e| {
                CertbotError::CommandFailed(format!(
                    "Failed to check certificate expiration: {}",
                    e
                ))
            })?;
        ...
        if output.status.success() {
            ...
            if let Some(date_part) = date_str.strip_prefix("notAfter=") {
                ...
                match DateTime::parse_from_str(date_part, "%b %d %H:%M:%S %Y GMT") {
                    ...
                    Err(_) => {
                        ...
                        match DateTime::parse_from_str(
                            ...
                            Ok(parsed_date) => Ok(parsed_date.with_timezone(&Utc)),
                            Err(e) => Err(CertbotError::CommandFailed(format!(
                ...
                Err(CertbotError::CommandFailed(
                    "Invalid openssl output format".to_string(),
                ))
            }
        } else {
            Err(CertbotError::CommandFailed(
...
mod tests {
    ...
    fn test_certificate_info_custom_path() {
        ...
        assert_eq!(
            cert_info.path_cert,
            "/custom/path/live/example.com/fullchain.pem"
        );
        assert_eq!(
            cert_info.path_key,
            "/custom/path/live/example.com/privkey.pem"
        );
    }
    ...
    fn test_cert_creation_example_com() {
        ...
        assert_eq!(
            std::mem::size_of_val(&_test_fn),
            std::mem::size_of::<fn(&str) -> Result<()>>()
        );
    }
    ...
    fn test_ensure_cert_example_com() {
        ...
        assert_eq!(
            std::mem::size_of_val(&_test_fn),
            std::mem::size_of::<fn(&str) -> Result<CertificateInfo>>()
        );
