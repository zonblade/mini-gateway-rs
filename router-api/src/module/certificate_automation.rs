...
use crate::module::certbot_runner::{CertbotConfig, CertbotManager, CertificateInfo};
...
impl CertificateAutomationManager {
    ...
    pub async fn ensure_certificate_and_save(
        &self,
        domain: &str,
        proxy_id: &str,
    ) -> Result<ProxyDomain> {
        info!(
            "Starting certificate automation for domain: {} (proxy: {})",
            domain, proxy_id
        );
        ...
        let existing_domain = domains
            .into_iter()
            .find(|d| d.sni.as_deref() == Some(domain));
        ...
        if let Some(domain_record) = &existing_domain {
            if !domain_record.tls_autron {
                info!(
                    "Domain {} has tls_autron=false, checking certificate origin",
                    domain
                );

                // Check if there are certificates in database
                if let (Some(ref cert_pem), Some(ref key_pem)) =
                    (&domain_record.tls_pem, &domain_record.tls_key)
                {
                    if !cert_pem.is_empty() && !key_pem.is_empty() {
                        ...
                        match self.certbot_manager.detect_certificate_origin(domain) {
                            Some(origin) if origin == "generated" => {
                                ...
                                let cert_info = CertificateInfo::new_with_config_dir(
                                    domain,
                                    &self.certbot_manager.get_config_dir(),
                                );
                    ...
                    match self.certbot_manager.detect_certificate_origin(domain) {
                        ...
                        None => {
                            info!(
                                "Domain {} has tls_autron=false and no certificates found",
                                domain
                            );
                        }
                    }
                ...
                warn!(
                    "Domain {} has tls_autron=false but no certificate data in database",
                    domain
                );
            ...
            info!(
                "Domain {} has tls_mode: {} (from DB: {:?})",
                domain, tls_mode, domain_record.tls_mode
            );
            ...
            info!(
                "Domain {} current manager is_production: {}, required mode: {}",
                domain, current_is_production, tls_mode
            );
            ...
            {
                info!(
                    "Domain {} requires different TLS mode ({}), creating appropriate manager",
                    domain, tls_mode
                );
                ...
                return appropriate_manager
                    .ensure_certificate_and_save_internal(domain, proxy_id)
                    .await;
            } else {
                info!(
                    "Domain {} will use current manager (production: {})",
                    domain, current_is_production
                );
            }
        }
        ...
        debug!(
            "Checking if certificate already exists for domain: {}",
            domain
        );
        match self.certbot_manager.certificate_exists(domain) {
            Ok(true) => {
                info!(
                    "Certificate files found for domain: {}, checking expiry",
                    domain
                );
                ...
                match self.check_certificate_expiry(&cert_info.path_cert) {
                        ...
                        if days_until_expiry > 30 {
                            ...
                            match self.validate_certificate_domain(&cert_info.path_cert, domain) {
                                Ok(true) => {
                                    ...
                                    return self
                                        .process_existing_certificate(cert_info, domain, proxy_id)
                                        .await;
                                }
                                Ok(false) => {
                                    warn!(
                                        "Certificate domain validation failed for {}, will renew",
                                        domain
                                    );
                                }
                                Err(e) => {
                            ...
                            info!(
                                "Certificate for domain {} expires in {} days, will renew",
                    ...
                    Err(e) => {
                        warn!(
                            "Could not check certificate expiry for domain {}: {}, will renew",
                            domain, e
                        );
                    }
                }
            ...
            Ok(false) => {
                info!(
                    "No existing certificate found for domain: {}, will create new",
                    domain
                );
            }
            Err(e) => {
        ...
        debug!(
            "Calling certbot to ensure certificate for domain: {}",
            domain
        );
        let cert_info = self
            .certbot_manager
        ...
        debug!(
            "Certificate paths - cert: {}, key: {}",
            cert_info.path_cert, cert_info.path_key
        );

        // Step 3: Read certificate and key files
        let cert_pem = std::fs::read_to_string(&cert_info.path_cert).map_err(|e| {
            CertificateAutomationError::CertificateParsing(format!(
                "Failed to read certificate file {}: {}",
                cert_info.path_cert, e
            ))
        })?;

        let key_pem = std::fs::read_to_string(&cert_info.path_key).map_err(|e| {
            CertificateAutomationError::CertificateParsing(format!(
                "Failed to read key file {}: {}",
                cert_info.path_key, e
            ))
        })?;
        ...
        info!(
            "Certificate automation completed successfully for domain: {}",
            domain
        );
    ...
    async fn ensure_certificate_and_save_internal(
        &self,
        domain: &str,
        proxy_id: &str,
    ) -> Result<ProxyDomain> {
        info!(
            "Starting certificate generation for domain: {} (proxy: {})",
            domain, proxy_id
        );
    ...
    pub async fn renew_due_certificates(&self) -> Result<(usize, usize)> {
        ...
        for domain in all_domains {
            ...
            let domain_name = match &domain.sni {
                ...
                _ => {
                    warn!(
                        "Skipping domain {} - no domain name (sni) specified",
                        domain.id
                    );
                    continue;
                }
            ...
            let needs_renewal = if let Some(expected_renew_str) = &domain.expected_renew {
                match DateTime::parse_from_rfc3339(expected_renew_str) {
                    Ok(expected_renew) => {
                        ...
                        debug!(
                            "Domain {} renewal due: {} (expected: {}, now: {})",
                ...
                debug!(
                    "Domain {} has no expected_renew date - treating as needing certificate",
                    domain_name
                );
                true
            };
            ...
            let missing_cert_data = domain.tls_pem.is_none()
                || domain.tls_key.is_none()
                || domain.tls_pem.as_ref().is_none_or(|s| s.is_empty())
                || domain.tls_key.as_ref().is_none_or(|s| s.is_empty());

            if missing_cert_data {
                debug!(
                    "Domain {} is missing certificate data in database, checking folder",
                    domain_name
                );
                ...
                if let Some(manager) = self.get_appropriate_manager_for_domain(&domain) {
                    match manager
                        .certbot_manager
                        .detect_certificate_origin(domain_name)
                    {
                        Some(origin) => {
                            info!(
                                "Domain {} has {} certificate in folder but missing from database",
                                domain_name, origin
                            );
            ...
            if needs_renewal || missing_cert_data {
                ...
                match self
                    .ensure_certificate_and_save(domain_name, proxy_id)
                    .await
                {
                    Ok(_) => {
                        successful += 1;
                        info!(
                            "Successfully renewed certificate for domain: {}",
                            domain_name
                        );
                    }
                    Err(e) => {
        ...
        info!(
            "Certificate renewal check completed. Successful: {}, Failed: {}",
            successful, failed
        );
    ...
    fn validate_certificate_domain(
        ...
        if domain_matches {
            debug!(
                "Certificate domain validation passed for {}: found domains {:?}",
                expected_domain, domains_found
            );
        } else {
            warn!(
                "Certificate domain validation failed for {}: found domains {:?}",
                expected_domain, domains_found
            );
        }
    ...
    fn get_appropriate_manager_for_domain(
        &self,
        domain: &ProxyDomain,
    ) -> Option<CertificateAutomationManager> {
        let default_tls_mode = "staging".to_string();
    ...
    async fn process_existing_certificate(
        &self,
        ...
        info!(
            "Successfully processed existing certificate for domain: {}",
            domain
        );
...
pub async fn generate_certificate_production(
    domain: &str,
    proxy_id: &str,
    email: Option<String>,
) -> Result<ProxyDomain> {
...
mod tests {
    ...
    fn test_certificate_automation_manager_creation() {
        ...
        assert_eq!(
            std::mem::size_of_val(&staging_manager),
            std::mem::size_of::<CertificateAutomationManager>()
        );

        let production_manager =
            CertificateAutomationManager::new_production(Some("test@example.com".to_string()));
        assert_eq!(
            std::mem::size_of_val(&production_manager),
            std::mem::size_of::<CertificateAutomationManager>()
        );
    }
}
