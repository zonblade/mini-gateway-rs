...
async fn auto_renewal_daemon(config: AutoRenewalConfig) {
    ...
    loop {
        ...
        log::info!(
            "Starting certificate expiry check (buffer: {} days)",
            config.buffer_days
        );
        ...
        match check_and_renew_expiring_certificates(&config).await {
            Ok((checked, renewed, failed)) => {
                log::info!(
                    "Certificate check completed: {} domains checked, {} renewed, {} failed",
                    checked,
                    renewed,
                    failed
                );
...
async fn check_and_renew_expiring_certificates(
    ...
    let auto_cert_domains: Vec<ProxyDomain> = all_domains
        .into_iter()
        .filter(|domain| domain.tls_autron && domain.sni.is_some() && domain.proxy_id.is_some())
        .collect();

    log::debug!(
        "Found {} domains with automatic certificates enabled",
        auto_cert_domains.len()
    );
                ...
                match DateTime::parse_from_rfc3339(expected_renew_str) {
                    Ok(expected_renew) => {
                        ...
                        if needs_renewal {
                            ...
                            log::debug!(
                                "Domain {} OK: expires {}, threshold {} (buffer: {} days)",
                                domain_name,
                                expected_renew,
                                renewal_threshold,
                                config.buffer_days
...
async fn renew_single_domain_with_retry(
    domain: &ProxyDomain,
    config: &AutoRenewalConfig,
    ...
    if email.is_none() || email.as_ref().map(|e| e.trim().is_empty()).unwrap_or(true) {
        log::error!(
            "Domain {} missing email - this should not happen due to validation",
            domain_name
        );
        return Err(format!(
            "Domain {} has no email configured for Let's Encrypt",
            domain_name
        )
        .into());
    }
    ...
    while attempt <= config.retry_attempts {
        log::debug!(
            "Certificate renewal attempt {} of {} for domain {}",
            attempt,
            ...
            .await
        {
            Ok(_) => {
                if attempt > 1 {
                    log::info!(
                        "Certificate renewal for {} succeeded on attempt {}",
                        domain_name,
                        attempt
                    );
                }
            ...
            Err(e) => {
                ...
                if attempt < config.retry_attempts {
                    let delay =
                        Duration::from_secs(config.retry_delay_seconds * 2_u64.pow(attempt - 1));
                    log::warn!(
                        "Certificate renewal attempt {} failed for {}, retrying in {:?}: {}",
                        attempt,
                        domain_name,
                        delay,
                    ...
                    log::error!(
                        "Certificate renewal for {} failed after {} attempts: {}",
                        domain_name,
                        config.retry_attempts,
                        last_error.as_ref().unwrap()
                    );
