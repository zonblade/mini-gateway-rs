use crate::api::users::helper::{can_modify_user, ClaimsFromRequest};
...
pub async fn init(req: HttpRequest, path: web::Path<String>) -> impl Responder {
    ...
    if !can_modify_user(&claims.sub, &claims.role, &user_id) {
        return HttpResponse::Forbidden()
            .json(serde_json::json!({"error": "You are not authorized to delete this user"}));
    }
    ...
    match db.query_one("SELECT id FROM users WHERE id = ?", [&user_id], |row| {
        row.get::<_, String>(0)
    }) {
        Ok(Some(_)) => {}
        Ok(None) => {
    ...
    // Delete the user
    match db.execute("DELETE FROM users WHERE id = ?", [&user_id]) {
        Ok(_) => {
            HttpResponse::Ok().json(serde_json::json!({"message": "User successfully deleted"}))
        }
        Err(err) => HttpResponse::InternalServerError()
            .json(serde_json::json!({"error": format!("Failed to delete user: {}", err)})),
    }
}
